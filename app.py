...
app.layout = dbc.Container(children=[
  ...
  dbc.Row([
    ...
    dbc.Col(children=[
      dbc.Row(dbc.Col(dcc.Slider(
        id='slider', min=0, max=40, step=2, value=6,
      ), width=12), id='dash_slider', style={"display": 'none'}),
  ...
  dbc.Row([
    dbc.Col(html.Button("disable image", id='img_stat', style={'width': "100%"}), width={'size': 2, 'offset': 9}),
...
@app.callback(Output('mask', 'image_content'),
              ...
              State('height', 'value'),
              State('width', 'value'))
def update_mask(trigger, image_content, img_stat_clicks, img_stat_label,
                json_data, mask_content, button_title, height, width):
  ctx = dash.callback_context
  component_id = ctx.triggered[0]['prop_id'].split('.')[0]
  if component_id == 'image':
    return image_content['props']['src'], no_update, None, None, None, 'disable image'
    ...
    if button_title == 'Reset':
      return image_content['props']['src'], "Save", no_update, no_update, no_update, no_update
    elif button_title == 'Save':
      mask = parse_jsonstring(json_data, (int(width), int(height)))
      mask_content = array_to_data_url((255 * (1 - mask)).astype(np.uint8))
      return mask_content, 'Reset', no_update, no_update, no_update, no_update
    else:
      raise PreventUpdate()
    ...
    if img_stat_label == 'disable image':
      style = {'display': 'none'}
      return no_update, no_update, style, style, style, 'enable image'
    elif img_stat_label == 'enable image':
      return no_update, no_update, None, None, None, 'disable image'
    else:
      raise PreventUpdate()
  else:
    raise PreventUpdate()
...
def do_compute(set_progress, n_clicks, prompt, negative_prompt, height, width, num_steps, temperature, guidance_scale,
               image_strength, image, mask, button_title, img_stat_label):
  if n_clicks is None:
    raise PreventUpdate()
